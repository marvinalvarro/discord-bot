...
function generateSetultahCard() {
    ...
    ctx.fillText("Daftar Ulang Tahun Kamu", width / 2, 195);

    ctx.strokeStyle = "rgba(76, 110, 245, 0.3)";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(width / 2 - 160, 212);
    ctx.lineTo(width / 2 + 160, 212);
    ctx.stroke();

    ctx.font = `500 18px ${FONT}`;
    ctx.fillStyle = "#5C6470";
    ctx.fillText("Daftarin sekali aja, biar bot yang inget-inget buat kamu tiap tahun!", width / 2, 244);
    ...
    steps.forEach((step, i) => {
        ...
        drawNumberBadge(ctx, stepsX, stepY, 22, i + 1, colors[i]);

        ctx.fillStyle = "#3A3A3A";
        ctx.font = `500 19px ${FONT}`;
        ctx.fillText(step, stepsX + 40, stepY + 7);
    ...
    ctx.fillText("FORMAT TANGGAL", width / 2, boxY + 32);

    ctx.fillStyle = "#4C6EF5";
    ctx.font = `700 30px ${FONT}`;
    ctx.fillText("DD - MM - YYYY", width / 2, boxY + 68);

    ctx.fillStyle = "#9AA0AA";
    ctx.font = `500 15px ${FONT}`;
    ctx.fillText("contoh: 17-08-2005", width / 2, boxY + 90);
